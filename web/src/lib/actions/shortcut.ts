...
/** Determines whether a KeyboardEvent should be ignored. The event will be ignored if:
...
export const shortcut = <T extends HTMLElement>(
  ...
  const { update: shortcutsUpdate, destroy } = shortcuts(node, [option]); //just call shortcuts by passing option into array
